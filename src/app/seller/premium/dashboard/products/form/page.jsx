...
export default function ProductFormLayout() {
  ...
  useEffect(() => {
    ...
    setAdVideo(currentProduct.adVideo || null)
  }, [currentProduct])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ---- ad video
      let uploadedAdVideo = null
      if (adVideo?.file && sellerType === 'premium_seller') {
        const v = await uploadVideoToCloudinary(adVideo.file)
        uploadedAdVideo = {
          url: v.secure_url,
      ...
      const dataToSubmit = {
        ...
        variantColumns,
        images: finalImages,
        ...(uploadedAdVideo ? { adVideo: uploadedAdVideo } : {}),
      }
      ...
      router.push('/seller/premium/dashboard/products')
    } catch (err) {
      console.error(err)
    }
  }
  ...
  return (
    <div className="mb-[40px]">
      ...
      <MultiImageUpload
        ...
        adVideo={adVideo}
        onAdVideoAdd={setAdVideo}
        onAdVideoRemove={() => setAdVideo(null)}
        maxImages={sellerType === 'premium_seller' ? 10 : 5}
        allowVideo={sellerType === 'premium_seller'}
      />
...
async function uploadVideoToCloudinary(file) {
  ...
  formData.append("file", file)
  formData.append("upload_preset", "ags_unsigned_upload")
  formData.append("folder", "ags/products/ads")
