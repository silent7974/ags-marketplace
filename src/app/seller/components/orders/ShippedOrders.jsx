...
export default function ShippedOrders() {
  ...
  const totalAmount = shippedOrders.reduce(
      ...
      {shippedOrders.length === 0 && (
        <div className="mt-16 flex flex-col items-center text-center">
          ...
          <p className="mt-1 text-[14px] text-black/50 max-w-[260px]">
            Orders currently on delivery will appear here.
          </p>
        </div>
      )}
          ...
          <div key={order._id}>
            ...
            <div className="mt-4 flex flex-col items-center gap-1 text-center">
              <p className="text-[12px] font-inter font-medium text-black/50">
                Courier has picked up the items. Delivery in progress...
              </p>

              <button
                className="flex gap-1 text-[10px] font inter text-[#000000]/50 "
              >
                Tracking available in 
                <span
                    onClick={() => setSelectedOrder(order)}
                    className="text-[#005770] underline"
                >
                    order details
                </span>
              </button>
