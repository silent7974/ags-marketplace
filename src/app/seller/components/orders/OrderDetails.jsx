...
export default function OrderDetails({ order, onClose }) {
  ...
  const pickupStatusMap = {
    pending: "Courier not assigned yet",
    processing: "Assigning courier",
    shipped: "Courier en route",
    delivered: "Delivered",
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Modal */}
        <motion.div
          className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-[24px]"
          initial={{ y: "100%" }}
          animate={{ y: 0 }}
          exit={{ y: "100%" }}
          transition={{
